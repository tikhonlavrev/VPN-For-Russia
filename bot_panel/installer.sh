...
surat=$(curl -sS https://raw.githubusercontent.com/tikhonlavrev/razresheniye/main/authbot | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -qO- https://raw.githubusercontent.com/tikhonlavrev/VPN-For-Russia/crud/ShellBot.sh >/etc/.maAsiss/.Shellbtsss
			}
